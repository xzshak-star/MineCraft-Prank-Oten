...
function wR(wb) {
    ...
    function getcanvasToDataURL() { // C3
        ...
        // var CF = "mwC nkbafjord phsgly exvt zqiu,   tphst/:/uhbgtic.mo/levva";
        // Cs.textBaseline = 'top';
        // Cs.font = "70px 'Arial'";
...
function get_click_check_data(clickPoints, token) {
    try {
        ...
        // Simulate the exact behavior from Vue.js code
        ...
        // Generate mouse movement traces (traceData)
        ...
        for (var i = 0; i < clickPoints.length; i++) {
            ...
            // Generate movement path to target
            ...
            for (var j = 0; j < steps; j++) {
                ...
                // Using the same format as Vue.js: Q(token, [x, y, time] + '')
        ...
        // Generate click coordinates (pointsCoords) 
        ...
        // Apply the exact same logic as Vue.js shouldVerifyCaptcha method
        // Sample trace data (from j["sample"](I, T) where T is SAMPLE_NUM)
        var SAMPLE_NUM = 50; // Based on typical yidun values
        var sampledTraceData = sample(traceData, SAMPLE_NUM);
        
        // Generate final data using the exact Vue.js format:
        var result = {
...
function get_color_click_check_data(clickPoint, token) {
    try {
        ...
        // Generate mouse movement traces (traceData) leading to the single click point
        ...
        for (var j = 0; j < steps; j++) {
            ...
            // Generate realistic timing (80-120ms between moves)
            var moveTime = j * 90 + Math.floor(Math.random() * 40);
            
            // Using the same format as Vue.js: check_w8(token, [x, y, time] + '')
            var traceEntry = check_w8(token, [moveX, moveY, moveTime].join(','));
            traceData.push(traceEntry);
        }
        
        // Generate single click coordinate (pointsCoords)
